...
&amp;maxfeatures=<xsl:value-of select="$maxFeatures"/>
   ...
   &amp;bbox=<xsl:value-of select="$bbox"/></xsl:if></xsl:variable><xsl:value-of select="translate(normalize-space($query),' ','')" disable-output-escaping="no"/><xsl:if test="ogc:Filter">&amp;filter=<xsl:value-of select="normalize-space($cFilterStr)"/></xsl:if></mb:QueryString></xsl:otherwise></xsl:choose></GetFeature></xsl:template><xsl:template match="text()|@*"/></xsl:stylesheet>
