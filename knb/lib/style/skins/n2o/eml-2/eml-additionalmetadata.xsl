...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="additionalmetadata">
    ...
    <xsl:param name="useindex"/>
    <div class="additionalmetadata">
      <h3>Additional Metadata <xsl:value-of select="$index"/></h3>
      <pre>
        <xsl:text>additionalMetadata&#xA;</xsl:text>
        <xsl:choose>
          <xsl:when test="$useindex">
            <xsl:variable name="node" select="additionalMetadata[position()=$index]"/>
            <xsl:apply-templates select="$node" mode="ascii-art"/>
          </xsl:when>
          <xsl:otherwise>
