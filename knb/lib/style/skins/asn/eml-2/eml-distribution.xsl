...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  <xsl:import href="eml-text.xsl" /><xsl:import href="eml-datatable-access.xsl"/>
  ...
  <div id="inMetadata" style="display:none">true</div>
  	<div id="PackageTitle" style="display:none">
		<xsl:value-of select="../../../../../title"/>
	</div>
    <xsl:variable name="URL" select="."/>
    <dl class="row">
      <dt><xsl:text>Data:</xsl:text></dt>
      <dd>
        <xsl:if test="$withHTMLLinks='1'">
          <div id="entityindex" style="display:none"><xsl:value-of select="$entityindex"/></div>
          <!--<a>-->
          <xsl:choose>
          <xsl:when test="starts-with($URL,'ecogrid')">
            ...
            <xsl:call-template name="datatableaccess">
		       	<xsl:with-param name="dUrl"><xsl:value-of select="$tripleURI"/><xsl:value-of select="$docID"/></xsl:with-param>
          ...
          <xsl:otherwise>
            <xsl:call-template name="datatableaccess">
		       	<xsl:with-param name="dUrl"><xsl:attribute name="href"><xsl:if test="not(contains(.,':/'))">http://</xsl:if><xsl:value-of select="$URL"/></xsl:attribute></xsl:with-param>
	   		</xsl:call-template>
