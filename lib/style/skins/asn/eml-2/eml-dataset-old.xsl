 ...
 <xsl:when test="references!=''">
  ...
  <xsl:for-each select="$references">
    <tr><td >
     ...
     <b><xsl:value-of select="./physical/objectName"/> </b></a>
    </td>
 </tr>
  </xsl:for-each>
</xsl:when>
...
Download File <img src="/knb/style/images/page_white_put.png" style="margin:0px 0px; padding:0px;" border="0" alt="download"/></a>    </xsl:when></xsl:choose>)
...
<xsl:template match="dataTable" name="link2"><li>
	
	<xsl:variable name="URL" select="./physical/distribution/online/url"/>

	<xsl:variable name="URL1" select="substring-after($URL, 'ecogrid://')"/>
    <xsl:variable name="dataDocID" select="substring-after($URL1, '/')"/>
    <xsl:variable name="docUrl"><xsl:value-of select="$tripleURI"/><xsl:value-of select="$dataDocID"/></xsl:variable>
	...
	<xsl:variable name="allowedlength" select="count(./physical/distribution/access/allow)"/>
	<xsl:choose>
	<xsl:when test="$allowed!=''">
	<b> ACCESS ONLY TO</b>
	<a id="linkDownload"><xsl:attribute name="onClick" ><![CDATA[getPrincipal("|]]><xsl:for-each select="$allowed"><xsl:variable name="permited" select="./principal"/><xsl:value-of select="$permited"/><![CDATA[|]]></xsl:for-each><![CDATA[","]]><xsl:value-of select="$docUrl"/><![CDATA[");]]></xsl:attribute>
	<xsl:attribute name="href">#</xsl:attribute> Download </a>
	...
	<div id="dialogDownload">
	  <h2>License</h2>
	  <p>bla bla bla</p>
	  <input id="licenseCheck" type="checkbox"/> <label for="licenseCheck"> Accept License Agreement</label>
	  <div><span id="mustAcceptLicense">You have to accept the license agreement to be able to download the data table.</span><a id="directDownloadLink">Download data table</a></div>
	</div>
	</xsl:when><!--
	<xsl:when test="$denied!=''">
		<xsl:variable name="dPermited" select="./principal"/>
		<xsl:variable name="dPermission" select="./permission"/>
		<xsl:when test="$dPermission!='read'" >
			<a id="linkDownload"><xsl:attribute name="onClick" ><![CDATA[getPrincipal("]]>null<![CDATA[","]]><xsl:value-of select="$docUrl"/><![CDATA[");]]></xsl:attribute>
			<xsl:attribute name="href">#</xsl:attribute> Download </a>
		</xsl:when>
	</xsl:when>-->
	<xsl:otherwise>
