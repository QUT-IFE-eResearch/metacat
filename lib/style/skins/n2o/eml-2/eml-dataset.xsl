    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <div>
          ...
          (<a><xsl:attribute name="href"><xsl:value-of select="$tripleURI"/><xsl:value-of select="$docid"/>&amp;displaymodule=entity&amp;entitytype=<xsl:value-of select="$type"/>&amp;entityindex=<xsl:value-of select="$index"/></xsl:attribute>View Metadata</a> 
          <xsl:if test="./physical/distribution/online/url">
            ...
            <a target="_blank">
              ...
              Download File 
              <img src="/knb/style/images/page_white_put.png" alt="download"/>
            </a>
          </xsl:if>)
        </div>
      </xsl:otherwise>
